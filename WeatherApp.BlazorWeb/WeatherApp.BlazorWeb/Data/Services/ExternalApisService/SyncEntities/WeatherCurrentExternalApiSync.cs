...
using WeatherApp.Shared.Models.ExternalApi.WeatherCurrent;

namespace WeatherApp.BlazorWeb.Data.Services.ExternalApisService.SyncEntities;

public class WeatherCurrentExternalApiSync : ExternalApiSyncService<long, WeatherCurrentBlazor, WeatherCurrentBlazor>,
    IExternalApiSyncService<long, WeatherCurrentBlazor, WeatherCurrentBlazor>
{
    ...
    ILogger<ExternalApiSyncService<long, WeatherCurrentBlazor, WeatherCurrentBlazor>> logger,
    IEntityBaseRepository<long, WeatherCurrentBlazor> entityBaseRepository,
    ...
    public async Task<IEnumerable<WeatherCurrentBlazor>> BeginRequest(string? parameters)
    {
        try
        {
            ...
            return new List<WeatherCurrentBlazor>();
        }
        catch (Exception ex)
        {
    ...
    private async Task<IEnumerable<WeatherCurrentBlazor>> AdditionalOperation(ApiWeatherRealtime apiResult)
    {
        var ret = new List<WeatherCurrentBlazor>();
        var createCounts = new Dictionary<string, int> { ["creatingWeatherCurrents"] = 0 };
        var createdWeatherCurrents = new List<WeatherCurrentBlazor>();
        ...
        if (apiResult != null && apiResult.Location != null && apiResult.Current?.Condition != null)
        {
            ...
            if (user != null)
            {
                var creatingWeatherCurrent = new WeatherCurrentBlazor
                {
                    AccountId = user.Id,
            ...
            await CustomRepository.CreateOrUpdateMultiAsync<long, WeatherCurrentBlazor>(createdWeatherCurrents);
