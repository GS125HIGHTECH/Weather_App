...
namespace WeatherApp.BlazorWeb.Data.Services.ExternalApisService.SyncEntities;

public class WeatherForecastExternalApiSync : ExternalApiSyncService<long, WeatherForecastBlazor, WeatherForecastBlazor>,
IExternalApiSyncService<long, WeatherForecastBlazor, WeatherForecastBlazor>
{
    ...
    ILogger<ExternalApiSyncService<long, WeatherForecastBlazor, WeatherForecastBlazor>> logger,
    IEntityBaseRepository<long, WeatherForecastBlazor> entityBaseRepository,
    ...
    public async Task<IEnumerable<WeatherForecastBlazor>> BeginRequest(string? parameters)
    {
        try
        {
            ...
            return new List<WeatherForecastBlazor>();
        }
        catch (Exception ex)
        {
    ...
    private async Task<IEnumerable<WeatherForecastBlazor>> AdditionalOperation(ApiWeatherForecast apiResult)
    {
        var ret = new List<WeatherForecastBlazor>();
        var createCounts = new Dictionary<string, int> { ["creatingWeatherForecasts"] = 0, ["creatingForecastDays"] = 0 };
        var createdWeatherForecasts = new List<WeatherForecastBlazor>();
        ...
        if (apiResult != null && apiResult.Location != null && apiResult.Current?.Condition != null && apiResult.Forecast?.ForecastDays != null)
        {
            ...
            if (user != null)
            {
                var creatingWeatherForecast = new WeatherForecastBlazor
                {
                    AccountId = user.Id,
                    ...
                    ForecastDays = apiResult.Forecast?.ForecastDays?.Select(day => new ForecastDayBlazor
                    {
                        Date = day.Date,
                        ...
                        ForecastHours = day.Hours?.Select(hour => new ForecastHourBlazor
                        {
            ...
            await CustomRepository.CreateOrUpdateMultiAsync<long, WeatherForecastBlazor>(createdWeatherForecasts);
